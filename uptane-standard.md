...
* An in-vehicle client on a primary ECU capable of verifying the signatures on all update metadata, potentially handling all server communication, and downloading updates on behalf of secondary ECUs
...
Please note, the subsequent sections concerning requirements for a primary does not prohibit implementing primary-capability on a component which does not communicate directly with Uptane repositories. This allows for implementations to have multiple ECUs within the vehicle performing functions equivalent to a primary.
